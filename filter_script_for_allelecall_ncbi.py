...
def badSerotypePrediction(line: list, filters: list):
    serotype_results = line[14].split('|')
...
def hasNoST(line: list):
    return line[15] in ['-', '', ' '] and line[4] != 'Complete'

def tooManyContigs(line: list):
    if line[5] in ['', ' '] or int(line[5]) > 150 and line[4] != 'Complete':
        return True
    else:
        return False

def intervalTotalAssemblyLen(line: list):
    if line[8] in ['', ' '] and line[4] != 'Complete':
        return True
    if float(line[8]) >= 1600000.0 or float(line[8]) <= 2400000.0 and line[4] == 'Complete':
        return False
    elif line[4] != 'Complete':
        return True

def isInEnaFile(line: list, ena_ids: list):
    if line[1] in ena_ids and line[4] != 'Complete':
        return True
    else:
        return False

def tooMuchMissingData(line: list):
    if int(line[10]) > 6 and line[4] != 'Complete':
        return True
    else:
        return False
...
def main(predictions_file: str, ena_file: str):
    target_dir = 'Filtered_fastas'
    lines_file = listTSVFile(predictions_file)
    ena_ids = [line[0] for line in listTSVFile(ena_file)]
    ...
    for line in lines_file:
        ...
        if isInEnaFile(line, ena_ids):
            putObservationForExclusion(line, "Is in ENA file")
        if tooMuchMissingData(line):
              putObservationForExclusion(line, "Missing Data > 6")
        if line[16] != 'X':
...
def parse_arguments():
    ...
    parser.add_argument('-e', '--ena', type=str,
                        required=True, dest='ena_file',
                        help='file with ena ids')
